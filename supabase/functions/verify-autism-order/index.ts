...
const log = (step: string, details?: Record<string, unknown>) => {
  console.log(`[VERIFY-AUTISM-ORDER] ${step}${details ? ` - ${JSON.stringify(details)}` : ""}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const metaOrderId = typeof session.metadata?.autism_order_id === "string"
      ? session.metadata.autism_order_id
      : null;
    ...
    if (isPaid && isComplete && metaMatches && amountMatches) {
      ...
      if (updErr) log("Failed to mark order paid", { error: updErr.message, orderId: existing.id });
      else log("Order marked paid", { orderId: existing.id });
    } else {
      log("Session not eligible to mark order paid", {
        isPaid, isComplete, metaMatches, amountMatches,
